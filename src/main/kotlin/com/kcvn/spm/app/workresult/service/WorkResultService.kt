    ...
    private fun mappingWorkResultResponse(workResults: List<WorkResult>): BasePagingResponse<WorkResultResponse> {
        ...
        response.data = workResults.map { x ->
            ...
            WorkResultResponse(
                ...
                summaryResultDate = if (x.summaryResultDate != null) DateTimeHelper.toUniversalTime(x.summaryResultDate!!) else null,
                workStartTime = if (x.workStartTime != null) DateTimeHelper.toUniversalTime(x.workStartTime!!) else null,
                workEndTime = if (x.workEndTime != null) DateTimeHelper.toUniversalTime(x.workEndTime!!) else null,
    ...
    fun exportExcel(request: WorkResultSearchRequest?, pageable: Pageable): BaseResponse<FileContentModel> {
        ...
        if (!workResultMapping.data.isNullOrEmpty()) {
            ...
            for (item in workResultMapping.data!!) {
                ...
                ExcelHelper.setCellValue(dataRow, 5, style, DateTimeHelper.toStringOrNull(item.summaryResultDate, "yyyy-MM-dd"))
                ExcelHelper.setCellValue(dataRow, 6, style, DateTimeHelper.toStringOrNull(item.workStartTime, "yyyy-MM-dd HH:mm"))
                ExcelHelper.setCellValue(dataRow, 7, style, DateTimeHelper.toStringOrNull(item.workEndTime, "yyyy-MM-dd HH:mm"))
                ...
                ExcelHelper.setCellValueInt(dataRow, 16, numberStyle, item.total, numberFormat)
                ExcelHelper.setCellValueInt(dataRow, 17, numberStyle, item.totalTapeQuantity, numberFormat)
                ExcelHelper.setCellValueInt(dataRow, 18, numberStyle, item.totalSheetQuantity, numberFormat)
                ExcelHelper.setCellValueInt(dataRow, 19, numberStyle, item.goodItemQuantity, numberFormat)
                ExcelHelper.setCellValueInt(dataRow, 20, numberStyle, item.goodTapeQuantity, numberFormat)
                ExcelHelper.setCellValueInt(dataRow, 21, numberStyle, item.goodSheetQuantity, numberFormat)
