...
import com.kcvn.spm.auth.service.RoleService
...
class RoleController(
    ...
    @GetMapping("/all")
    @PreAuthorize("hasAuthority(T(com.kcvn.spm.auth.security.EPermission).VIEW_ROLE.value) || hasRole('ADMIN')")
    fun getAllRoles(
    ...
    @PostMapping("/create")
    @PreAuthorize("hasAuthority(T(com.kcvn.spm.auth.security.EPermission).CREATE_ROLE.value) || hasRole('ADMIN')")
    fun createRole(@RequestBody request: @Valid RoleRequest?): ResponseEntity<*> {
        ...
        return if (role == null) {
            ...
            ResponseEntity<MessageResponse>(
                MessageResponse(
                    messageSource.getMessage("action.failed", null, LocaleContextHolder.getLocale()),
                    role
                ),
                HttpStatus.CREATED
            )
    ...
    @PreAuthorize("hasAuthority(T(com.kcvn.spm.auth.security.EPermission).UPDATE_ROLE.value) || hasRole('ADMIN')")
    fun updateRole(
    ...
    @PreAuthorize("hasAuthority(T(com.kcvn.spm.auth.security.EPermission).DELETE_ROLE.value) || hasRole('ADMIN')")
