    ...
    fun update(data: CompletionRateProcessProduct): CompletionRateProcessProduct? {
        ...
        context.transaction { configuration ->
            ...
            result = transactionalContext
                ...
                .set(COMPLETION_RATE_PROCESS_PRODUCT.UPDATED_DATE, Instant.now().atOffset(ZoneOffset.UTC))
    ...
    fun getPaginatedCompletionRateProcessesProduct(
        ...
        if (search != null) {
            if (!search.productNameShortCut.isNullOrEmpty()) {
                val lowerProductNameShortCutSearch = DSL.lower(search.productNameShortCut!!.trim())
            ...
            if (!search.processCode.isNullOrEmpty()) {
                val lowerProcessCodeSearch = search.processCode!!.trim()
