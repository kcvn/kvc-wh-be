        ...
        if (requestErr.isNotEmpty()) {
            ...
            for (item in requestErr) {
                if(item.messageErrs.isNullOrEmpty()) continue
                    ...
                    dataRow.createCell(2).setCellValue(item.layerCode)
                    dataRow.getCell(2).cellStyle = style

                    dataRow.createCell(3).setCellValue(item.processConvertCode)
                    dataRow.getCell(3).cellStyle = style

                    dataRow.createCell(4).setCellValue(item.processInventoryCode)
                    dataRow.getCell(4).cellStyle = style

                    dataRow.createCell(5).setCellValue(item.processStatisticCode)
                    dataRow.getCell(5).cellStyle = style

                    dataRow.createCell(6).setCellValue(item.messageErrs?.joinToString(separator = "; "))
                    dataRow.getCell(6).cellStyle = style
                }
            }
        }
